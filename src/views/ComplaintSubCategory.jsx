    ...
    const ListItem: React.FC<ListItemProps> = React.memo(
        ...
        return (
            <Animated.View style={[
                ...
                <TouchableOpacity activeOpacity={1} onPress={navigate_complaint.bind(this, item.id, item)} style={{ backgroundColor: colors.theme_bg_three, padding: 10, marginLeft: 10, marginRight: 10, marginTop:5, marginBottom:5, borderRadius: 10 }}>
                    <View style={{ margin: 10 }}>
                        <Text ellipsizeMode='tail' style={{ color: colors.theme_fg_two, fontSize: f_s, fontFamily:normal }}>{item.complaint_sub_category_name}</Text>
                        <View style={{ margin:3 }} />
                        <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_s, fontFamily: regular ,textAlign:'justify'}}>
                            {item.short_description} 
                        </Text>
                    </View>
                </TouchableOpacity>
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={[styles.header]}>
                ...
                <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
                    <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: regular }}>{complaint_category_name}</Text>
                </View>
            </View>
            <FlatList
                data={data}
